...
INSERT INTO orders (id, product_id, buyer_id, seller_id, amount, payment_method, delivery_method, status, escrow_hold, confirmed_at, released_at, created_at, paid_at)
SELECT 12, 3, 2, 1, 45000, 'wallet', 'kazpost', 'completed', 'released', NOW() - INTERVAL 2 DAY, NOW() - INTERVAL 2 DAY, NOW() - INTERVAL 6 DAY, NOW() - INTERVAL 6 DAY
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM orders WHERE id = 12);
...
INSERT IGNORE INTO reviews (order_id, author_id, subject_id, role, rating, body, created_at) VALUES
(12, 2, 1, 'as_seller', 5, 'Отличный курс, материал полезный. Jason — топ преподаватель.', NOW() - INTERVAL 2 DAY),
(12, 1, 2, 'as_buyer', 5, 'Студент ответственный, всё вовремя.', NOW() - INTERVAL 2 DAY);
