            ...
            <?php elseif ($tab === 'photo'): ?>
                ...
                <form method="post" action="<?= ProductHelper::url('/profile/avatar') ?>" enctype="multipart/form-data" class="flex flex-col items-center gap-5 py-6">
                    <label class="relative group cursor-pointer">
                        ...
                        <span class="absolute inset-[13%] rounded-full bg-ink-900/55 opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-white text-xs font-bold uppercase tracking-wide z-[2]"><?= htmlspecialchars(t('profile.change')) ?></span>
                    </label>
            ...
            <?php elseif ($tab === 'lots'): ?>
                ...
                <form method="post" action="<?= $editingGig ? ProductHelper::url('/profile/gigs/' . (int) $editing['id'] . '/update') : ($editing ? ProductHelper::url('/profile/lots/' . $editing['id'] . '/update') : ProductHelper::url('/profile/store')) ?>" enctype="multipart/form-data" id="lot-create-form" class="space-y-4 mb-8 p-5 rounded-2xl border border-black/[0.06] dark:border-white/10 bg-brand-50/30 dark:bg-white/[0.03]">
                    ...
                    (function () {
                        ...
                        const publishServiceLabel = <?= json_encode(ProductHelper::isServiceListingFreePromo()
                            ? t('profile.publish_service_free')
                            : t('profile.publish_service', ['amount' => \App\Models\Wallet::formatMoney(ProductHelper::serviceListingFee())])) ?>;
