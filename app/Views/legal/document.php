...
<section class="max-w-3xl mx-auto space-y-6 fade-up pb-10">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-2 leading-relaxed whitespace-pre-line"><?= htmlspecialchars(t($docKey . '.lead')) ?></p>
        <p class="text-xs text-gray-400 mt-2 whitespace-pre-line"><?= htmlspecialchars(t($docKey . '.effective')) ?></p>
    </div>
    ...
    <div class="rounded-[24px] border border-black/[0.06] dark:border-white/10 bg-white/90 dark:bg-white/[0.04] shadow-soft p-5 sm:p-6 space-y-2">
        ...
        <p class="text-sm text-ink-700/80 dark:text-gray-300 leading-relaxed whitespace-pre-line"><?= htmlspecialchars(t($docKey . '.contacts_body')) ?></p>
        <p class="text-sm">
