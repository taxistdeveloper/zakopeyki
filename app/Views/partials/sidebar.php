...
<aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-[272px] glass border-r border-black/[0.06] dark:border-white/10 flex flex-col h-full transition-transform duration-300 transform -translate-x-full lg:translate-x-0 shadow-soft">
    <div class="relative h-[72px] flex items-center justify-center px-5 border-b border-black/[0.06] dark:border-white/10">
        <a href="<?= ProductHelper::url('/') ?>" class="flex items-center flex-shrink-0 group transition hover:opacity-90">
            <img src="<?= ProductHelper::url('/public/assets/img/logo-light.png') ?>" alt="Zakopeyki" width="220" height="70" class="h-11 w-auto object-contain dark:hidden">
            <img src="<?= ProductHelper::url('/public/assets/img/logo-dark.png') ?>" alt="Zakopeyki" width="220" height="70" class="h-11 w-auto object-contain hidden dark:block">
        </a>
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto scrollbar-hide">
        ...
        <?php
            $sidebarCartCount = 0;
            try {
                $sidebarCartCount = \App\Services\Cart::count();
            } catch (\Throwable $e) {
                $sidebarCartCount = 0;
            }
        ?>
        ...
        <a href="<?= ProductHelper::url('/cart') ?>" class="nav-item w-full flex items-center gap-3 px-3.5 py-2.5 text-sm rounded-xl transition <?= navClass('cart', $nav) ?>">
            <span class="flex items-center gap-3 flex-1 min-w-0"><?= $navIcon('bag') ?> <span><?= htmlspecialchars(t('nav.cart')) ?></span></span>
            <?php if ($sidebarCartCount > 0): ?>
                <span id="sidebar-cart-badge" class="min-w-[1.25rem] h-5 px-1.5 rounded-full bg-accent-500 text-white text-[10px] font-bold flex items-center justify-center"><?= $sidebarCartCount > 99 ? '99+' : $sidebarCartCount ?></span>
            <?php else: ?>
                <span id="sidebar-cart-badge" class="hidden min-w-[1.25rem] h-5 px-1.5 rounded-full bg-accent-500 text-white text-[10px] font-bold flex items-center justify-center">0</span>
            <?php endif; ?>
        </a>
        <?php if ($user): ?>
        <?php
