...
<div class="space-y-5">
    <div class="flex items-start gap-3">
        ...
        <div class="min-w-0">
            ...
            <p class="text-sm text-gray-500 mt-1"><?= htmlspecialchars(t('verify.lead')) ?></p>
        </div>
    </div>
    ...
    <?php else: ?>
        <form method="post" action="<?= ProductHelper::url('/profile/verify-listing') ?>" id="listing-verify-form" class="space-y-4">
            ...
            <?php elseif ($hasSavedIin): ?>
                <div>
                    <label class="block text-xs font-bold mb-1"><?= htmlspecialchars(t('profile.iin')) ?></label>
            ...
            <?php else: ?>
                <div>
                    <label class="block text-xs font-bold mb-1" for="listing-verify-iin"><?= htmlspecialchars(t('profile.iin')) ?> <span class="text-red-500">*</span></label>
                    <input type="text" name="iin" id="listing-verify-iin" inputmode="numeric" maxlength="12" autocomplete="off" required pattern="\d{12}" class="<?= $input ?>" placeholder="000000000000">
                    <p id="listing-verify-iin-error" class="hidden text-[11px] text-red-600 mt-1"><?= htmlspecialchars(t('flash.aml_iin_invalid')) ?></p>
